...
> Rows marked **(verify)** still need confirmation against the official
> "What's New" page for that release before the dependent feature tags are
> considered final.
...
| Delphi XE3 | — | 2012 | **Record helpers for intrinsic types**; helper for simple types (verify exact scope) |
...
| Delphi 10.1 | Berlin | 2016 | **Custom-managed string helper reach**; (verify) |
...
| Delphi 13.1 | Florence | 2026 | Arm64EC compiler & tooling — *no new core-language syntax* (verify) |
...
- **Record helpers for intrinsic types**: commonly attributed to XE3 — confirm
  before finalising any `7.x` string-helper feature tags.
- **ARC** (🧪): lived only on the mobile/NextGen compilers (XE4 → 10.3),
  fully **removed in 10.4**. Treat all ARC-specific syntax as historical.
- **Delphi 13.1**: the headline items (Arm64EC, FMX Style Designer, Win11 VCL
  styles) are tooling/library — verify whether any *language* item shipped in
  the point release before tagging anything `13.1`.
